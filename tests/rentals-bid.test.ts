...
import fixture from "./fixtures/rentals-bid.fixture.json" with { type: "json" };
...
async function rig(body: unknown) {
 ...
 return { urls, call: (args: Record<string, unknown>) => client.callTool({ name: "rentals_by_bid", arguments: args }),
 close: async () => { await client.close(); await server.close(); } };
}
it("requires bid and limit and preserves the captured legacy rental response", async () => {
 const r = await rig(fixture.body);
 try {
  expect((await r.call({ limit: 3 })).isError).toBe(true);
  expect((await r.call({ bid: "fixture_bid" })).isError).toBe(true);
  expect(r.urls).toHaveLength(0);
  const result = await r.call({ bid: "fixture_bid", limit: 3, offset: 0 });
  expect(result.isError).not.toBe(true);
  expect(result.structuredContent).toEqual(fixture.body);
  expect(r.urls).toHaveLength(1);
  expect(r.urls[0]!.pathname).toBe("/delegation-rental/rentals/bid/fixture_bid");
...
it("bounds rental records and rejects a malformed record beyond the return cap", async () => {
 ...
 try {
  const result = await r.call({ bid: "fixture_bid", limit: 100 });
  expect(result.isError).not.toBe(true);
 ...
 try {
  expect((await bad.call({ bid: "fixture_bid", limit: 100 })).isError).toBe(true);
